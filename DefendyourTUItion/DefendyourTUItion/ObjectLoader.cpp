...
/* To enable object model:
In Enemy.h add header "ObjectLoader.hpp"
In Enemy.cpp void Enemy::initModel() change all the code with :
...
In Rederer.cpp void Renderer::drawModel(std::shared_ptr<Model> model, glm::mat4 transform)
change the last line : "glDrawArrays(GL_TRIANGLES, 0, model->getTriangleCount() * 3);" to
...
In Camera.cpp: m_cameraSpeed = 10.f; (i reduced the speed from 50 to 10)
